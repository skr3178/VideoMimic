...
def setup_camera_follow(
    ...
    camera_wxyz: Optional[np.ndarray] = None,  # Shape (T,4) - optional camera wxyz
    ...
    target_ema_alpha: float = 0.005, # Smoothing factor for target position (0 < alpha <= 1)
    camera_ema_alpha: float = 0.005, # Smoothing factor for camera position (0 < alpha <= 1, only used if camera_positions is not None)
    ...
    Args:
        ...
        camera_wxyz: Optional array of shape (T,4) containing camera wxyz.
    ...
    if camera_positions is not None:
        ...
        def update_camera_for_target(client: viser.ClientHandle, t: int):
            ...
            client.camera.position = camera_positions[t]
            client.camera.wxyz = camera_wxyz[t]
